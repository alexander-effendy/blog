...
interface MyDialogProps {
  ...
  handleEditPost: (postNum: Number) => void;
};

const MyDialog: React.FC<MyDialogProps> = ({ postId, postTitle, postBody, isOpen, closeModal, handleTitleChange, handleBodyChange, handleEditPost }) => {
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="flex flex-col">
                    <section className="flex flex-col mt-5">
                      <label>Title:</label>
                      <input value={postTitle} onChange={handleTitleChange} className="rounded-lg h-[30px] w-full" />
                    </section>
                    
                    <label className="mt-5">Body:</label>
                    <textarea value={postBody} onChange={handleBodyChange} className="rounded-lg" rows={10} />
                  </div>
